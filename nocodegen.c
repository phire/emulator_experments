...
#define addFlags if(switchyThing & 1) { c = ((signed)((Rn ^ shifter) ^ r) < 0) ^ ((signed)((Rn ^ shifter) & ~(Rn ^ shifter)) < 0); v = (signed)((Rn ^ r) & ~ (Rn ^ shifter)) < 0; }
#define subFlags if(switchyThing & 1) { c = ((signed)((Rn ^ shifter) ^ r) < 0) ^ ((signed)((Rn ^ shifter) & (Rn ^ shifter)) < 0); v = (signed)((Rn ^ r) & (Rn ^ shifter)) < 0; }
/*
	...
	for(;;) {
		...
		if(conditional == 0xe || conditions[cpsr >> 28] & 1 << conditional){// Conditional execution
			...
			case 0x37 ... 0x3f: { // Thanks gcc for your lovely range operator
				if(1) {};
				uint8_t opcode = (switchyThing >> 1) & 0xf;
